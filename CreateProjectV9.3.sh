...
EOF
        ...
        {
            # Common top-level dependencies with pinned versions
            echo "fastapi==0.116.0"
            echo "uvicorn==0.35.0"
            ...
            if [[ "$component" == "auth" ]]; then
                echo "resend==2.10.0"
                # This installs pydantic, email-validator, and dnspython with compatible versions
                echo "pydantic[email]==2.11.7"
            else
                # For app, database, storage - no email validation needed
        ...
        } | sudo tee "$req_file" > /dev/null
        sudo chown "$APP_USER:$APP_USER" "$req_file"
        sudo chmod 664 "$req_file"
        log_ok "Generated requirements.txt for '$component'."

        log_ok "Installing Python dependencies for '$component'..."
        sudo -u "$APP_USER" bash -c "cd '$APP_PATH/$component' && python3 -m venv venv && source venv/bin/activate && pip --no-cache-dir install --upgrade pip > /dev/null && pip --no-cache-dir install -r requirements.txt > /dev/null"
